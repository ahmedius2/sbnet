...
struct TileShared {
    scalar_t* tile_;
    int rr_, ss_, cc_;
    __forceinline__ __device__ void init(scalar_t* ext_shmem, int rr, int ss, int cc)
    {
        tile_ = ext_shmem;
        rr_ = rr;
        ss_ = ss;
    ...
    __forceinline__ __device__ scalar_t& val(int hh, int ww, int cc) {
        constexpr int O = 0;
        return tile_[(hh)*ss_*(cc_+O) + (ww)*(cc_+O) + cc];
    }
};

template<typename scalar_t, int RR1, int SS, int CC1>
struct TileLocal {
    __forceinline__ __device__ void init(scalar_t* ext_shmem, int rr, int ss, int cc)
    {
        assert(rr == RR1);
...
template<typename scalar_t, int tNTHREADS, typename tHb, typename tH1b, typename tWb, typename tC, typename tC1, typename tTranspose, typename tTile>
__device__ void blockGatherTiled_t(
    const scalar_t* x,                        // NHWC
    // currently only supporting single bin
    const int16_t* activeBlockIndices,
    ...
    )
{
	...
	scalar_t *shmem = reinterpret_cast<scalar_t *>(my_smem);
...
#pragma unroll 4
    for (int cr1sStrided = 0; cr1sStrided < R1S*C1; cr1sStrided += tNTHREADS)
    {
        int cr1s = cr1sStrided + threadIdx.x;
            ...
            scalar_t readVal = tile.val(tileH, intraBw, tileC);
...
template<typename scalar_t, int tNTHREADS, typename tHb, typename tH1b, typename tWb, typename tC, typename tC1,
    typename tAdd, typename tTranspose, typename tAtomic, typename tTile>
__device__ void blockScatterTiled_t(
    const scalar_t* x,                        // blockDim.x*tHb*tWb*tC - input tensor
    // currently only supporting single bin
    const int16_t* activeBlockIndices, // same indices as for blockGather
    // variable size output but conservative worst-case memory alloc (0% sparsity, all blocks active)
    scalar_t* y,                              // NHWC
    const int N, const int H, const int W, // C=tC is templated
    const int bOffsH0, const int bOffsW0,  // generally negative - first block element offset for correct padding
...
template<typename scalar_t, int tNTHREADS, int RR, int RR1, int SS, int CC1, bool TR>
__global__ void blockGatherTiled0(
    const scalar_t* x,                               // NHWC
    // currently only supporting single bin
    const int16_t* activeBlockIndices, // result
...
{
    blockGatherTiled_t<scalar_t, tNTHREADS, getConst<RR>, getConst<RR1>, getConst<SS>, getVar, getConst<CC1>, getConst<TR>, TileShared<scalar_t>>(
...
template<typename scalar_t, int tNTHREADS, int RR, int RR1, int SS, int CC1, bool ADD, bool TR, bool ATOMIC>
...
{
    blockScatterTiled_t<
        scalar_t, tNTHREADS, getConst<RR>, getConst<RR1>, getConst<SS>, getVar,
        getConst<CC1>, getConst<ADD>, getConst<TR>, getConst<ATOMIC>, TileShared<scalar_t>>
    (
        x, activeBlockIndices, y, N, H, W, bOffsH0, bOffsW0, bStrH, bStrW,
...
template<typename scalar_t, int tNTHREADS>
...
{
    blockGatherTiled_t<scalar_t, tNTHREADS, getVar, getVar, getVar, getVar, getVar, getVar, TileShared<scalar_t>>(
        x, activeBlockIndices, y, N, H, W, bOffsH0, bOffsW0, bStrH, bStrW,
...
{
    blockScatterTiled_t<scalar_t, tNTHREADS, getVar, getVar, getVar, getVar, getVar, getVar, getVar, getVar, TileShared<scalar_t>>(
        x, activeBlockIndices, y, N, H, W, bOffsH0, bOffsW0, bStrH, bStrW,
